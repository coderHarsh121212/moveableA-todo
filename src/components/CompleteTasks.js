  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex gap-4 items-center ">
        ...
        <p className="text-xs">{plannedTasks.length} Opened Tasks</p>
      </div>

      <div className="flex flex-col gap-2 w-full  rounded text-xs">
        {plannedTasks.map((e) => (
          <div
            className="flex justify-between border-2 p-3 rounded"
            key={e.title}
          >
            <div className="flex gap-3 h-auto w-1/2 items-center">
              <input type="checkbox" checked={e.checked}></input>
            ...
            <div className="flex justify-between  items-center gap-3">
              <p>people</p>
              <p onClick={() => deleteTask(e.id, "completeTasks")}>
                <DeleteIcon />
              </p>
            </div>
          </div>
